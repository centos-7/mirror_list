...
# jukka.lehto@hetzner.com - 2018.9.25 - modified for wiping disks without confirmation+nvme support
# jukka.lehto@hetzner.com - 2018.10.25 - duct tape for hardcoded directories
...
if [ -z "$(get_disks)" ] ; then
  ...
  # 01.09.2018 10:55 jukka.lehto~
  # don't care if only 1 disk
...
# 01.09.2018 10:56 jukka.lehto~
# going to wipe disks anyway, don't care about partitions
...
for disk in $(get_disks | cut -d: -f1) ; do
  ...
  echo "0,0,0" | sfdisk -q $disk 1>/dev/null 2>/dev/null
done
