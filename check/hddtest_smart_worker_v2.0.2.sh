...
check_hdd_health() {
  ...
  SMART_HEALTH=$(smartctl $4 -H $disk | egrep "FAILED!|SAVE ALL DATA|FAILING_NOW")
...
check_smart_values() {
  ...
  SMART_FAILING_NOW=$(smartctl $4 -A $disk | sed 's/^[ \t]*//' | grep '^[0-9]' | grep NOW)
  ...
  SMART_LBA_Error=0
  SMART_EXTEND=$(smartctl $4 -x $disk | sed 's/^[ \t]*//' | egrep '^[0-9]{1,3}\ [A-Z]|^Error\ [1-9]|^Lifetime|^Min/Max.*Limit')
...
start_selftest() {
  ...
  # check test result
  #
  selftest_result=$(smartctl $4 -l selftest $disk)
...
write_result_log() {
  ...
  if [ -z "$FAILED" ]; then
    echo "Disk: $serials OK" >> $LOGDIR/hddtest-$serials.log
    send hddsmart-result "OK" "Finished (Mode: $MODE, Device: $disk_wo_dev); [$STARTTIME - $ENDTIME]" "$serials"
  else
    echo "Disk: $serials FAIL" >> $LOGDIR/hddtest-$serials.log
    echo "hdd_smart_test:error:hdd:too many badblocks at $disk_wo_dev" >> $ROBOT_LOGFILE
...
# check for SSD
#
if [ "$(smartctl $4 -a $disk | egrep -i "SSD|MZ7WD480HAGM|MZ7WD240HAFV")" ]; then
